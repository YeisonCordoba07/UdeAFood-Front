  ...
  return (
    <div className="flex gap-5 flex-wrap p-5">
      {tiendas.map((tienda) => (
        <div
          key={tienda.id}
          className="flex flex-col w-[290px] h-[380px] p-4 border border-gray-100 rounded-lg font-bold gap-4 items-start shadow-lg"
        >
          {/* Imagen de la tienda */}
          <Image
            src={
              tienda.foto ? `data:image/jpeg;base64,${tienda.foto}` : "/all.jpg"
            }
            alt={tienda.nombre}
            width={300}
            height={300}
